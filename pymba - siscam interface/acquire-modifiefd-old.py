...
# STATUS: Live view possible if called from IDLE, crashes at some point (runtime errors...) ; 
#			if called via standard shell crashes when image passed to queue in acqthread, or when image to be plotted. (see remarks at these points) 
...
#		- VCam.snap() returns what kind of data-type?
#		- AcquireThread : Opens, Sets timing, starts taking images, saves to queue, closes ('with closing')
#		- Crashing not due to buffer size (always 1 or 0 when checked)
#
...
######## ---------------------- End new section ------------------
    ...
    def OnTestButton(self, event):##### ATTENTION TODO remove this function
        print 'AVT imaging mode', self.imaging_mode_AVT
        print 'AVT exposure', self.timing_AVT.exposure
        print 'AVT timing external', self.timing_AVT.external
    ...
    def OnSetTiming(self, event):
        ...
        if event.Id == self.ID_TimingAVTSettings:
            dialog = self.create_timing_dialog(exposure=self.timing_AVT.exposure,
                                               repetition=self.timing_AVT.repetition)
            res = dialog.ShowModal()
            if res == wx.ID_OK:
                exp, rep = dialog.GetResults()
                self.timing_AVT.exposure, self.timing_AVT.repetition = exp, rep
            dialog.Destroy()
...
#### ---------- End new section ----------------			
    ...
    def create_timing_dialog(self, exposure, repetition=None):

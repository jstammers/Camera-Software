...
import imagefile
loadimg = imagefile.load_image_giacomo
...
class ImgAppAui(wx.App):
    ...
    roiK = [ROI(100, 200, 100, 200), ROI(100, 200, 100, 200), ROI(0, 640, 0, 480)] #list of ROIs for all imaging settings
    roiRb = [ROI(100, 200, 100, 200), ROI(100, 200, 100, 200), ROI(0, 640, 0, 480)]


    imaging_parlist = [{'Rb': imagingpars.ImagingParsVertical(), 'K': imagingpars.ImagingParsVertical()},
                       {'Rb': imagingpars.ImagingParsHorizontal(), 'K': imagingpars.ImagingParsHorizontal()},
                       {'Rb': imagingpars.ImagingParsBlueFox(), 'K': imagingpars.ImagingParsBlueFox()}]

    for i in range(len(imaging_parlist)):
        imaging_parlist[i]['Rb'].mass = 87.0 * 1.66e-27
        imaging_parlist[i]['K'].mass = 41.0 * 1.66e-27
    ...
    def OnChangeExpansionTime(self, event):
        ...
        elif id == self.ID_ExpansionTimeRb:
    ...
    def OnChangeMaximumOpticalDensity(self, event):
        ...
        if id == self.ID_OpticalDensityMaxK:
            self.K.update()
        elif id == self.ID_OpticalDensityMaxRb:
            self.Rb.update()
 ...
 | wx.aui.AUI_MGR_ALLOW_FLOATING
        ...
        fit_menu.AppendRadioItem(
            self.ID_FitKGauss,
            "K Gauss fit",
            "perform 2d Gauss fit for Kalium",
            )
        fit_menu.AppendRadioItem(
            self.ID_FitKGaussSym,
            "K symmetric Gauss fit",
            "perform symmetric (sx = sy) 2d Gauss fit for Kalium",
            )
        fit_menu.AppendRadioItem(
        ...
        fit_menu.AppendRadioItem(
            self.ID_FitKBimodal,
            "K bimodal fit",
            "perform 2d fit of Gaussian + Thomas-Fermi distribution for Kalium",
            )
        ...
        fit_menu.AppendRadioItem(
            self.ID_FitKTF,
            "K Thomas-Fermi fit",
            "perform 2d Thomas-Fermi fit distribution for Kalium",
            )
        ...
        imgK, imgRb = loadimg(self.imagefilename)
        imgK = ma.array(imgK, mask= ~ numpy.isfinite(imgK))
        imgRb = ma.array(imgRb, mask= ~ numpy.isfinite(imgRb))
        
        self.K = ImgPanel(self.frame, 'K', imgK, self.roiK[0], fitting.Gauss2d(self.imaging_pars['K']))
        ...
        self.mgr.AddPane(self.K, wx.aui.
                         AuiPaneInfo().Name("K").Caption("Kalium").
        ...
        for panel in [self.savedimageK, self.savedimageRb]:
    ...
    def UpdateResults(self):
        self.results.UpdateResults({'K': self.K.fitpars, 'Rb': self.Rb.fitpars})
